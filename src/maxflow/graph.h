...
public:
	...
	int get_arc_cardinal_num(arc_id a); // if a=get_first_arc() then returns 0; for the next one returns 1, and so on.

	// we can also query arcs insident to a specific node.
	arc_id get_first_incident_arc(node_id i); // may return NULL
	arc_id get_next_incident_arc(arc_id a); // may return NULL
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline int Graph<captype,tcaptype,flowtype>::get_arc_cardinal_num(arc* a) 
{
	return (int)(a - arcs); 
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_first_incident_arc(node_id i)
{
	return nodes[i].first;
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_next_incident_arc(arc* a) 
{
	return a->next; 
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_reverse_arc(arc* a) 
{
	return a->sister; 
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::get_arc_end(arc* a, node_id& j)
{
	assert(a >= arcs && a < arc_last);
	j = (node_id) (a->head - nodes);
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::add_trcap(node_id i, tcaptype trcap)
{
	assert(i>=0 && i<node_num);
	nodes[i].tr_cap += trcap;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline tcaptype Graph<captype,tcaptype,flowtype>::add_trcap_X(node_id i, tcaptype trcap)
{
	assert(i>=0 && i<node_num);
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::reset_flow()
{
	flow = 0;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename Graph<captype,tcaptype,flowtype>::termtype Graph<captype,tcaptype,flowtype>::what_segment(node* i, termtype default_segm)
{
	if (i->parent)
	{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline captype Graph<captype,tcaptype,flowtype>::remove_crossing_arcs(node_id _i)
{
	node* i = nodes + _i;
	arc** a_ptr;
	captype sum = 0;
	...
	{
		for (a_ptr=&i->first; *a_ptr; )
		{
			arc* a = *a_ptr;
			if (what_segment(a->head) == SOURCE) a_ptr = &a->next;
			else
			{
				sum += a->sister->r_cap;
				*a_ptr = a->next;
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::restore_crossing_arcs()
{
	arc* a;
	for (a=arcs; a<arc_last; a+=2)
	{
		if (a->next != a && (a+1)->next != (a+1)) continue;
		node* i = (a+1)->head;
		node* j = a->head;
		a->next = i->first;
		i->first = a;
